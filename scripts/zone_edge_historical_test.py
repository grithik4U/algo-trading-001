...
def _load_klines(provider: BinancePublicData, start: datetime, end: datetime) -> pd.DataFrame:
    """Page Binance 1m klines because the API caps each request at 1000 rows."""
    cursor = start
...
def _bars_to_profile_trades(bars: pd.DataFrame) -> pd.DataFrame:
    """Create deterministic pseudo-trades from 1m OHLCV for profile structure.

    Volume is split across OHLC prices. Buy/sell classification is inferred
    from candle direction. This is deliberately labeled approximate; it is
    intended for multi-day structural validation, not execution research.
    """
    if bars.empty:
...
def _build_profiles_from_bars(bars: pd.DataFrame):
    ...
    while cursor + WINDOW_SIZE <= end:
        ...
        if len(window) >= 30:
            ...
            profiles.append((pd.Timestamp(cursor, tz="UTC"), pd.Timestamp(window_end, tz="UTC"), profile))
        cursor += STEP
    return profiles
        ...
        for i, (ts, row) in enumerate(bars.iterrows()):
            ...
            if touched:
                ...
                if bar_high > high and bar_low < low and inside:
                    ...
                    inside_streak, outside_streak, last_state = 1, 0, "SWEEP"
                    continue
                if inside:
                    inside_streak += 1
                    outside_streak = 0
...
def _baseline(bars, events, horizon):
    ...
    for pos in range(len(bars) - horizon):
        ...
        raw = float(bars["close"].iloc[pos + horizon]) - entry
...
def main():
    ...
    for horizon in HORIZONS:
        base = _baseline(bars, events, horizon)
        rows = []
        for event in events:
            metrics = _future_metrics(bars, event, horizon)
            if metrics is None or base.get(event.direction) is None:
                continue
            move, mfe, mae = metrics
            rows.append((event, move, base[event.direction], mfe, mae, _outcome(mfe, mae)))
        print(f"\n=== {horizon}M EDGE ===")
        ...
        print(f"baseline anchors: UP={base['UP']:.2f} DOWN={base['DOWN']:.2f}" if base['UP'] is not None and base['DOWN'] is not None else "baseline anchors: insufficient data")
